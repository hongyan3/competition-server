  ...
  <div class="enrty-member-conatiner">
    ...
    <a-table :columns="columns" :data="data.userList" :pagination="{
      ...
      <template #optional="{ record }">
        <a-button status="danger" type="primary" @click="deleteUserHandle(record.id)">删除</a-button>
      </template>
    </a-table>
    <!--  -->
    <a-modal v-model:visible="addModalVisible" title="添加成员" @cancel="addModalVisible = false" @ok="addUserHandle">
      <a-form :model="userForm">
        <a-form-item field="userName" label="昵称">
          <a-input v-model="userForm.userName" placeholder="请输入昵称"></a-input>
        </a-form-item>
        <a-form-item field="userAccount" label="账号">
          <a-input v-model="userForm.userAccount" placeholder="请输入账号"></a-input>
        </a-form-item>
        <a-form-item field="password" label="密码">
          <a-input v-model="userForm.password" placeholder="请输入密码" type="password"></a-input>
        </a-form-item>
...
import {UserAdminControllerService as userService, type UserVO,} from '@/api';
