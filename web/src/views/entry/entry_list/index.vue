    ...
    <div class="enrty-list-conatiner">
        ...
        <a-table :columns="columns" :data="data.entryList" :pagination="{
            ...
            <template #optional="{ record }">
                ...
                <a-button type="primary" style="margin-left: 5px;" @click="toMemberHandle(record.id)">成员</a-button>
                <a-button type="primary" style="margin-left: 5px;" @click="deleteEntryHandle(record.id)" status="danger">删除</a-button>
            </template>
        </a-table>
        <!--  -->
...
import {
    UserControllerService as userService,
...
const addEntryHandle = () => {
    entryService.addEntryUsingPost1(entryForm).then((res) => {
        if (res.code == 0) {
