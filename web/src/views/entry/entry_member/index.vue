        ...
        <a-modal v-model:visible="addModalVisible" title="添加成员" @cancel="addModalVisible = false" @ok="addMemberHandle">
            <a-form :model="memberForm">
                <a-form-item field="sourceName" label="昵称/账户">
                    <a-dropdown>
                        <a-input :style="{ width: '320px' }" placeholder="请输入用户昵称" allow-clear
                            @input="searchUser" v-model="memberForm.memberId" />
                        <template #content>
                            <a-doption @click="selectUserHandle(item.id as number)" :style="{width: '320px'}" v-for="(item, index) in data.userList">
