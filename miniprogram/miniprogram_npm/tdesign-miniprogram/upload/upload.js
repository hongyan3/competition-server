    ...
    constructor() {
        ...
        this.methods = {
            ...
            chooseMedia(mediaType) {
                ...
                wx.chooseMedia(Object.assign(Object.assign({count: max === 0 ? 9 : max, mediaType}, config), {
                    success: (res) => {
                        const files = [];
                        res.tempFiles.forEach((temp) => {
                            const {size, fileType, tempFilePath, width, height, duration, thumbTempFilePath} = temp,
                                res = __rest(temp, ["size", "fileType", "tempFilePath", "width", "height", "duration", "thumbTempFilePath"]);
            ...
            chooseMessageFile(mediaType) {
                ...
                wx.chooseMessageFile(Object.assign(Object.assign({
                    ...
                    success: (res) => {
                        const files = [];
                        res.tempFiles.forEach((temp) => {
                            const {size, type: fileType, path: tempFilePath} = temp,
                                res = __rest(temp, ["size", "type", "path"]);
                            if (sizeLimit && size > sizeLimit) {
                                wx.showToast({
                                    icon: 'none',
                                    title: `${fileType === 'image' ? '图片' : '视频'}大小超过限制`
